...
public class AugmentableFeatureVector extends FeatureVector
{
	...
	public AugmentableFeatureVector (Alphabet dict, PropertyList pl, boolean binary,
		...
		while (pl.numericIterator().hasNext()) {
			pl.numericIterator().nextProperty();
			//System.out.println ("AugmentableVector ("+dict.size()+") adding "+iter.getKey()+" "+iter.getNumericValue());
			int index = dict.lookupIndex (pl.numericIterator().getKey(), growAlphabet);
			if (index >= 0)
				add (index, pl.numericIterator().getNumericValue());
		}
	}
	...
	public void add (FeatureVector fv, String prefix)
	{
		...
		for (int loc = 0; loc < fv.numLocations (); loc++) {
			...
			Object otherName = otherDict.lookupObject (idx);

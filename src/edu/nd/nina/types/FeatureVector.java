...
/**
 * A subset of an {@link cc.mallet.types.Alphabet} in which each element of the
 * subset has an associated value. The subset is represented as a
 * {@link cc.mallet.types.SparseVector}
 * <p>
 * A SparseVector represents only the non-zero locations of a vector. In the
 * case of a FeatureVector, a location represents the index of an entry in the
 * Alphabet that is contained in the FeatureVector.
 * <p>
 * To loop over the elements of a feature vector, one loops over the consecutive
 * integers between 0 and the number of locations in the feature vector. From
 * these locations one can cheaply obtain the index of the entry in the
 * underlying Alphabet, the entry itself, and the value in this feature vector
 * associated the entry.
 * <p>
 * A SparseVector (or FeatureVector) can be sparse or dense depending on whether
 * or not an array if indices is specified at construction time. If the
 * FeatureVector is dense, the mapping from location to index is the identity
 * mapping.
 * <p>
 * The associated value of an element in a SparseVector (or FeatureVector) can
 * be a double or binary (0.0 or 1.0), depending on whether an array of doubles
 * is specified at contruction time.
 * 
 * @see SparseVector
 * @see Alphabet
 * 
 ...
 */
public class FeatureVector extends SparseVector implements AlphabetCarrying {
	private static Logger logger = Logger.getLogger(FeatureVector.class
			.getName());

	Alphabet dictionary;

	protected FeatureVector(Alphabet dict, int[] indices, double[] values,
			int capacity, int size, boolean copy, boolean checkIndicesSorted,
	...
	/**
	 * Create non-binary vector, possibly dense if "featureIndices" or possibly
	 * sparse, if not
	 */
	...
	public static int[] getObjectIndices(String[] entries, Alphabet dict,
	...
	public FeatureVector(Alphabet dict, String[] keys, double[] values) {
	...
	private static int[] indicesWithConjunctions(FeatureVector fv,
			Alphabet newVocab, int[] conjunctions) {
		...
		assert (v.size() <= newVocab.size()) : "fv.vocab.size=" + v.size()
				+ " newVocab.size=" + newVocab.size();
			...
			if (conjunctions[ci] == fv.indices[i]) {
				for (int j = 0; j < fv.indices.length; j++) {
					if (conjunctions[ci] != fv.indices[j]) {
						int index = newVocab.lookupIndex(FeatureConjunction
								.getName(v, conjunctions[ci], fv.indices[j]));
						if (index == newVocab.size() - 1 && index % 3 == 0)
							logger.info("New feature "
									+ newVocab.lookupObject(index));
						if (index != -1) // this can be -1 if
											// newVocab.growthStopped
	...
	private static int[] indicesWithConjunctions(FeatureVector fv,
			Alphabet newVocab, FeatureSelection fsNarrow,
		...
		for (int i = 0; i < length; i++) {
			...
			for (int j = 0; j < length; j++) {
				...
				// && !FeatureConjunction.featuresOverlap (v, fv.indices[i],
				// fv.indices[j]))
				) {
					int index = newVocab.lookupIndex(FeatureConjunction
							.getName(v, fv.indices[i], fv.indices[j]));
	...
	/**
	 * New feature vector containing all the features of "fv", plus new features
	 * created by making conjunctions between the features in "conjunctions" and
	...
	/**
	 * Construct a new FeatureVector, selecting only those features in fs, and
	 * having new (presumably more compact, dense) Alphabet.
	 */
	public static FeatureVector newFeatureVector(FeatureVector fv,
			Alphabet newVocab, FeatureSelection fs) {
		assert (fs.getAlphabet() == fv.dictionary);
		if (fv.indices == null) {
			throw new UnsupportedOperationException(
					"Not yet implemented for dense feature vectors.");
		}

		// this numLocations() method call ensures that
		// AugmentableFeatureVectors have been compressed
		...
		for (int index = 0; index < fvNumLocations; index++) {
			if (fs.contains(fv.indices[index])) {
				try {
					indices[size] = newVocab
							.lookupIndex(fv.dictionary
									.lookupObject(fv.indices[index]), true);
				} catch (Exception e) {
		...
		return new FeatureVector(newVocab, indices, values, size, size, true,
				true, false);
	}
	...
	public ConstantMatrix cloneMatrixZeroed() {
		...
		if (indices == null)
			...
			return new FeatureVector(dictionary, newIndices,
					new double[values.length], values.length, values.length,
					false, false, false);
		}
	}
	...
	// CPAL - added this to output Feature vectors to a text file in a simple
	// format
	public boolean toSimpFile(String FileName, int curdocNo, boolean printcounts) {
		...
		if (values == null) {
			...
			for (int i = 0; i < indicesLength; i++) {
				...
				if (dictionary == null)
					...
					// System.out.println
					// ("FeatureVector toString: i="+i+" index="+indices[i]);
		...
		try {
			FileWriter out = new FileWriter(myfile, true); // true -> append to
															// the file
			out.write(str);
			out.close();
		} catch (IOException e) {
			System.err
					.println("Feature Vector exception when trying to print a file");
		}
	...
	public int location(String entry) {
		if (dictionary == null)

...
public class IndexedSparseVector extends SparseVector {
	// private static Logger logger =
	// Logger.getLogger(SparseVector.class.getName());

	transient private int[] index2location;

	public IndexedSparseVector(int[] indices, double[] values, int capacity,
			int size, boolean copy, boolean checkIndicesSorted,
	...
	public ConstantMatrix cloneMatrixZeroed() {
		...
		IndexedSparseVector sv = new IndexedSparseVector(newIndices,
				new double[values.length], values.length, values.length, false,
				false, false);
	...
	private void setIndex2Location() {
		// System.out.println
		// ("IndexedSparseVector setIndex2Location indices.length="+indices.length+" maxindex="+indices[indices.length-1]);
		assert (indices != null);
	...
	public final void setValue(int index, double value) {
		if (index2location == null)
			setIndex2Location();
		int location = index < index2location.length ? index2location[index]
				: -1;
		if (location >= 0)
	...
	public int location(int index) {
		// No test for indices == null, for this is not allowed in an
		// IndexedSparseVector
		if (index2location == null)
			setIndex2Location();
		if (index >= index2location.length)

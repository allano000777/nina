...
import edu.nd.nina.graph.load.Noop;
import edu.nd.nina.graph.load.Pipe;
import edu.nd.nina.math.Randoms;
import edu.nd.nina.util.MatrixOps;

/**
 * A list of machine learning instances, typically used for training or testing
 * of a machine learning algorithm.
 * <p>
 * All of the instances in the list will have been passed through the same
 * {@link cc.mallet.pipe.Pipe}, and thus must also share the same data and
 * target Alphabets. InstanceList keeps a reference to the pipe and the two
 * alphabets.
 * <p>
 * The most common way of adding instances to an InstanceList is through the
 * <code>add(PipeInputIterator)</code> method. PipeInputIterators are a way of
 * mapping general data sources into instances suitable for processing through a
 * pipe. As each {@link cc.mallet.types.Instance} is pulled from the
 * PipeInputIterator, the InstanceList copies the instance and runs the copy
 * through its pipe (with resultant destructive modifications) before saving the
 * modified instance on its list. This is the usual way in which instances are
 * transformed by pipes.
 * <p>
 * InstanceList also contains methods for randomly generating lists of feature
 * vectors; splitting lists into non-overlapping subsets (useful for test/train
 * splits), and iterators for cross validation.
 * 
 * @see Instance
 * @see Pipe
...
public class InstanceList extends ArrayList<Instance> implements
		Iterable<Instance>, AlphabetCarrying {
	// private static Logger logger =
	// Logger.getLogger(InstanceList.class.getName());

	private static final long serialVersionUID = 1L;

	HashMap<Instance, Double> instWeights = null;
	// This should never be set by a ClassifierTrainer, it should be used in
	// conjunction with a Classifier's FeatureSelection
	// Or perhaps it should be removed from here, and there should be a
	// ClassifierTrainer.train(InstanceList, FeatureSelection) method.
	 ...
	 * pipe (for example using instanceList.addThruPipe); but this is not
	 * required. This InstanaceList will obtain its dataAlphabet and
	 * targetAlphabet from the pipe. It is required that all Instances in this
	 * InstanceList share these Alphabets.
	 * 
	 * @param pipe
	 *            The default pipe used to process instances added via the
	 *            addThruPipe methods.
	 * @param capacity
	 *            The initial capacity of the list; will grow further as
	 *            necessary.
	 */
	...
	/**
	 * Construct an InstanceList with initial capacity of 10, with given default
	 * pipe. Typically Instances added to this InstanceList will have gone
	 * through the pipe (for example using instanceList.addThruPipe); but this
	 * is not required. This InstanaceList will obtain its dataAlphabet and
	 * targetAlphabet from the pipe. It is required that all Instances in this
	 * InstanceList share these Alphabets.
	 * 
	 * @param pipe
	...
	/**
	 * Construct an InstanceList with initial capacity of 10, with a Noop
	 * default pipe. Used in those infrequent circumstances when Instances
	 * typically would not have further processing, and objects containing
	 * vocabularies are entered directly into the <code>InstanceList</code>; for
	 * example, the creation of a random <code>InstanceList</code> using
	 * <code>Dirichlet</code>s and <code>Multinomial</code>s.</p>
	 * 
	 * @param dataAlphabet
	...
	public InstanceList shallowClone() {
		...
		// Should we really be so shallow as to not make new copies of these
		// following instance variables? -akm 1/2008
	...
	/**
	 * Adds to this list every instance generated by the iterator, passing each
	 * one through this InstanceList's pipe.
	 */
	...
	public void addThruPipe(Iterator<Instance> ii) {
		...
		while (pipedInstanceIterator.hasNext()) {
			...
			// pipedInstanceIterator.next().getName());
		}
	}
	 ...
	 * InstanceList's pipe.
	 * <p>
	 * If several instances are to be added then accumulate them in a
	 * List\<Instance\> and use <tt>addThruPipe(Iterator<Instance>)</tt>
	 * instead.
	 */
	 ...
	 * @deprecated Use trainingset.addThruPipe (new
	 *             Instance(data,target,name,source)) instead.
	 */
	@Deprecated
	public boolean add(Object data, Object target, Object name, Object source,
		...
		Iterator<Instance> ii = pipe
				.newIteratorFrom(new SingleInstanceIterator(inst));
		if (ii.hasNext()) {
	 ...
	 * 
	 * @return <code>true</code>
	 * @deprecated Use trainingset.add (new Instance(data,target,name,source))
	 *             instead.
	 */
	@Deprecated
	public boolean add(Object data, Object target, Object name, Object source) {
	 ...
	 * the InstanceList's pipe. The alphabets of this Instance must match the
	 * alphabets of this InstanceList.
	 * 
	 * @return <code>true</code>
	 */
	public boolean add(Instance instance) {
		...
		if (!Alphabet.alphabetsMatch(this, instance)) {
			...
			sb.append("Instance: ["
					+ (data_alphabet == null ? null : data_alphabet.size())
					+ ", "
			...
			// ("Alphabets don't match: Instance: "+
		...
		// Once it is added to an InstanceList, generally-speaking, the Instance
		// shouldn't change.
		// There are exceptions, and for these you can instance.unlock(), then
		// instance.lock() again.
		instance.lock();
	...
	public boolean add(Instance instance, double instanceWeight) {
		...
		if (!ret)
			// If for some reason a subclass of InstanceList refuses to add this
			// Instance, be sure not to do the rest.
			return ret;
		if (instanceWeight != 1.0) { // Default weight is 1.0 for everything not
										// in the HashMap.
			if (instWeights == null)
				instWeights = new HashMap<Instance, Double>();
			else if (instWeights.get(instance) != null)
				throw new IllegalArgumentException(
						"You cannot add the same instance twice to an InstanceList when it has non-1.0 weight.  "
	...
	public boolean addAll(int index, Collection<? extends Instance> c) {
		throw new IllegalStateException(
				"addAll(int,Collection) not supported by InstanceList.n");
	}
	...
	@Deprecated
	// Remove this. It seems like too specialized behavior to be implemented
	// here.
	...
	protected InstanceList cloneEmptyInto(InstanceList ret) {
		ret.instWeights = null; // Don't copy these, because its empty!
								// instWeights == null ? null :
								// (HashMap<Instance,Double>)
								// instWeights.clone();
		// xxx Should the featureSelection and perLabel... be cloned?
		// Note that RoostingTrainer currently depends on not cloning its
		// splitting.
	...
	public InstanceList[] split(double[] proportions) {
		return split(new java.util.Random(System.currentTimeMillis()),
				proportions);
	}
	 ...
	 * 
	 * @param proportions
	 *            A list of numbers corresponding to the proportion of elements
	 *            in each returned sublist. If not already normalized to sum to
	 *            1.0, it will be normalized here.
	 * @return one <code>InstanceList</code> for each element of
	 *         <code>proportions</code>
	 */
	public InstanceList[] splitInOrder(double[] proportions) {
		...
		for (int i = 0; i < maxind.length; i++) {
			ret[i] = this.cloneEmpty(); // Note that we are passing on
										// featureSelection here.
			if (i > 0)
				maxind[i] += maxind[i - 1];
		}
		for (int i = 0; i < maxind.length; i++) {
			// Fill maxind[] with the highest instance index to go in each
			// corresponding returned InstanceList
	 ...
	 * every <code>m</code>th element of this list, starting with the first. The
	 * second list contains all remaining elements.
	 */
	...
	/**
	 * Returns an <code>InstanceList</code> of the same size, where the
	 * instances come from the random sampling (with replacement) of this list
	 * using the instance weights. The new instances all have their weights set
	 * to one.
	 */
	 ...
	 * Returns an <code>InstanceList</code> of the same size, where the
	 * instances come from the random sampling (with replacement) of this list
	 * using the given weights. The length of the weight array must be the same
	 * as the length of this list The new instances all have their weights set
	 * to one.
	 */
	// added by Gary - ghuang@cs.umass.edu
	public InstanceList sampleWithWeights(java.util.Random r, double[] weights) {
		...
		for (int i = 0; i < size(); i++) {
			if (weights[i] < 0)
				throw new IllegalArgumentException(
						"weight vector must be non-negative");
	...
	// added by Fuchun
	/**
	 * Replaces the <code>Instance</code> at position <code>index</code> with a
	 * new one.
	 */
	...
	public void setFeatureSelection(FeatureSelection selectedFeatures) {
		if (selectedFeatures != null && selectedFeatures.getAlphabet() != null // xxx
																				// We
																				// allow
																				// a
																				// null
	...
	/**
	 * Sets the "target" field to <code>null</code> in all instances. This makes
	 * unlabeled data.
	 */
	...
	/**
	 * Sets the "source" field to <code>null</code> in all instances. This will
	 * often save memory when the raw data had been placed in that field.
	 */
	 ...
	 * <code>file</code>. If the string value of <code>file</code> is "-", then
	 * deserialize from {@link System.in}.
	 */
	public static InstanceList load(File file) {
		try {
			...
			e.printStackTrace();
			throw new IllegalArgumentException(
					"Couldn't read InstanceList from file " + file);
		}
	}

	/**
	 * Saves this <code>InstanceList</code> to <code>file</code>. If the string
	 * value of <code>file</code> is "-", then serialize to {@link System.out}.
	 */
	public void save(File file) {
		try {
			...
			e.printStackTrace();
			throw new IllegalArgumentException(
					"Couldn't save InstanceList to file " + file);
		}
	}
	 ...
	 * based on nfolds.
	 */
	public class CrossValidationIterator implements Iterator<InstanceList[]> {
		int nfolds;
		InstanceList[] folds;
		int index;
		 ...
		 * 
		 * @return A pair of lists, where <code>InstanceList[0]</code> is the
		 *         larger split (training) and <code>InstanceList[1]</code> is
		 *         the smaller split (testing)
		 */
	...
	/**
	 * Returns the pipe through which each added <code>Instance</code> is
	 * passed, which may be <code>null</code>.
	 */
	public Pipe getPipe() {
		return pipe;
	}
	...
	/**
	 * Change the default Pipe associated with InstanceList. This method is very
	 * dangerous and should only be used in extreme circumstances!!
	 */
	...
	public Alphabet getDataAlphabet() {
		...
		assert (pipe == null || pipe.getDataAlphabet() == null || pipe
				.getDataAlphabet() == dataAlphabet);
		return dataAlphabet;
	}
	...
	public Alphabet getTargetAlphabet() {
		...
		assert (pipe == null || pipe.getTargetAlphabet() == null || pipe
				.getTargetAlphabet() == targetAlphabet);
	...
	// I'm not sure these methods best belong here. On the other hand it is easy
	// to find and centrally located here. -AKM Jan 2006
	public void hideSomeLabels(double proportionToHide, Randoms r) {
		for (int i = 0; i < this.size(); i++) {
			if (r.nextBoolean(proportionToHide)) {
				...
				instance.unLock();
				if (instance.getProperty(TARGET_PROPERTY) != instance
						.getTarget())
	...
	public void unhideAllLabels() {
		for (int i = 0; i < this.size(); i++) {
			...
			if (instance.getTarget() == null
					&& (t = instance.getProperty(TARGET_PROPERTY)) != null) {
				instance.unLock();

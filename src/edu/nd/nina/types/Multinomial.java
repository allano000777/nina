...
import edu.nd.nina.math.Dirichlet;
import edu.nd.nina.math.Randoms;

/**
 * A probability distribution over a set of features represented as a
 * {@link cc.mallet.types.FeatureVector}. The values associated with each
 * element in the Multinomial/FeaturVector are probabilities and should sum to
 * 1. Features are indexed using feature indices - the index into the underlying
 * Alphabet - rather than using locations the way FeatureVectors do.
 * <p>
 * {@link cc.mallet.types.Multinomial.Estimator} provides a subhierachy of ways
 * to generate an estimate of the probability distribution from counts
...
public class Multinomial extends FeatureVector {
	...
	// "size" is the number of entries in "probabilities" that have valid values
	// in them;
	// note that the dictionary (and thus the resulting multinomial) may be
	// bigger than size
	// if the dictionary is shared with multiple estimators, and the dictionary
	// grew
	// due to another estimator.
	private static double[] getValues(double[] probabilities,
			Alphabet dictionary, int size, boolean copy, boolean checkSum) {
		double[] values;
	...
	public Multinomial(double[] probabilities, Alphabet dictionary,
		...
		super(dictionary, getValues(probabilities, dictionary, size, copy,
				checkSum));
	}
	...
	public double probability(Object key) {
		if (dictionary == null)
			throw new IllegalStateException(
					"This Multinomial has no dictionary.");
	...
	/**
	 * A Multinomial in which the values associated with each feature index fi
	 * is Math.log(probability[fi]) instead of probability[fi]. Logs are used
	...
	public static class Logged extends Multinomial {
		...
		public Logged(double[] probabilities, Alphabet dictionary,
			...
			this(probabilities, dictionary,
					(dictionary == null ? probabilities.length : dictionary
							.size()), areLoggedAlready);
		}
	...
	/**
	 * A hierarchy of classes used to produce estimates of probabilities, in the
	 * form of a Multinomial, from counts associated with the elements of an
	 * Alphabet.
	 * 
	 ...
	 * Alphabet changes size between calls. It does not contain any means of
	 * generating probability estimates; various means of estimating are
	...
	public static abstract class Estimator implements Cloneable {
		...
		public Estimator(int size) {
			this(new double[size > minCapacity ? size : minCapacity], size,
					null);
		}
		...
		protected void ensureCapacity(int index) {
			// assert (dictionary == null); // Size is fixed if dictionary
			// present?
			if (index > size)
				size = index;
			if (counts.length <= index) {
				int newLength = ((counts.length < minCapacity) ? minCapacity
						: counts.length);
		...
		// xxx Add "public void increment (Object key, double count)", or is it
		// too dangerous?
		...
		public void increment(FeatureVector fv, double scale) {
			...
			for (int fvi = 0; fvi < fv.numLocations(); fvi++)
				// Originally, the value of the feature was not being taken into
				// account here,
				// so words were only counted once per document! - gdruck
				// increment (fv.indexAtLocation(fvi), scale);
				increment(fv.indexAtLocation(fvi),
						scale * fv.valueAtLocation(fvi));
		}
	...
	/**
	 * An Estimator in which probability estimates in a Multinomial are
	 * generated by adding a constant m (specified at construction time) to each
	 * count before dividing by the total of the m-biased counts.
	 */
	...
	public static class MEstimator extends Estimator {
		...
		public Multinomial estimate() {
			double[] pr = new double[dictionary == null ? size : dictionary
					.size()];
			if (dictionary != null) {
				ensureCapacity(dictionary.size() - 1); // side effect: updates
														// size member
			}
			double sum = 0;
			for (int i = 0; i < pr.length; i++) {
				// if (dictionary != null) System.out.println
				// (dictionary.lookupObject(i).toString()+' '+counts[i]);
				pr[i] = counts[i] + m;
				sum += pr[i];
			}
			for (int i = 0; i < pr.length; i++)
	...
	/**
	 * An MEstimator with m set to 0. The probability estimates in the
	 * Multinomial are generated by dividing each count by the sum of all
	 * counts.
	 */
	...
	/**
	 * An MEstimator with m set to 1. The probability estimates in the
	 * Multinomial are generated by adding 1 to each count and then dividing
	 * each 1-biased count by the sum of all 1-biased counts.
	 */

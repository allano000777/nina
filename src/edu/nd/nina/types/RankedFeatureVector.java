...
public class RankedFeatureVector extends FeatureVector {
	...
	public RankedFeatureVector(final Alphabet dict, final int[] indices,
			final double[] values) {
	...
	private static double[] subArray(final double[] a, final int begin,
			final int length) {
	...
	public RankedFeatureVector(final Alphabet dict,
			final AugmentableFeatureVector v) {
	...
	protected void setRankOrder(final int extent, final boolean reset) {
		int sortExtent;
		// Set the number of cells to sort, making sure we don't go past the
		// max.
	...
	// added by Limin Yao, rank the elements ascendingly, the smaller is in the
	// front
	...
	public int getMaxValuedIndexIn(final FeatureSelection fs) {
		...
		// xxx Make this more efficient! I'm pretty sure that Java BitSet's can
		...
		// System.out.println
		// ("RankedFeatureVector.getMaxValuedIndexIn feature="
	...
	public Object getObjectAtRank(final int rank) {
		setRankOrder(rank);
		return dictionary.lookupObject(getIndexAtRank(rank)); // was return
	...
	public double getValueAtRank(int rank) {
		...
		if (rank >= rankOrder.length) {
			...
			System.err.println("rank larger than rankOrder.length. rank = "
					+ rank + "rankOrder.length = " + rankOrder.length);
		}
	...
	/**
	 * Prints a human-readable version of this vector, with features listed in
	 * ranked order.
	 * 
	 * @param out
	 *            Stream to write to
	...
	public void set(final int i, final double v) {
		throw new UnsupportedOperationException(
				RankedFeatureVector.class.getName() + " is immutable");
	}

	/*
	 * public interface Factory { public RankedFeatureVector
	 * newRankedFeatureVector (InstanceList ilist); }
	 * 
	 * public interface PerLabelFactory { public RankedFeatureVector[]
	 * newRankedFeatureVectors (InstanceList ilist); }
	 */

	private static class EntryWithOriginalIndex implements
			Comparable<EntryWithOriginalIndex> {
		private final double _value;
		private final int _originalIndex;

		public EntryWithOriginalIndex(final double value,

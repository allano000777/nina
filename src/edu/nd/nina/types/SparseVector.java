 ...
 * 
 * When you create a SparseVector, you pass in a list of indices. These are the
 * only elements of the vector you will be allowed to change. The rest are fixed
 * at 0.
 * 
 * The interface to Sparse vector uses the concept of a location, which is an
 * integer in the range 0..numLocations which can be mapped to the index (and
 * value) of a non zero element of the vector.
 * 
 * A SparseVector can be sparse or dense depending on whether or not an array if
 * indices is specified at construction time. If the SparseVector is dense, the
 * mapping from location to index is the identity mapping. The type of the value
 * an element in a SparseVector (or FeatureVector) can be double or binary (0.0
 * or 1.0), depending on whether an array of doubles is specified at contruction
 * time.
 * 
 * @author Andrew McCallum <a
 *         href="mailto:mccallum@cs.umass.edu">mccallum@cs.umass.edu</a>
 */
public class SparseVector implements ConstantMatrix {
	/**
	 * If the vector is sparse, then both indices and values are sparse. Indices
	 * into these arrays are called ``locations'' in the below. The indices[]
	 * array maps locations to indices of the (virtual) dense array that's being
	 * represented. value[] maps locations to values.
	 */
	...
	/**
	 * If "indices" is null, the vector will be dense. If "values" is null, the
	 * vector will be binary. The capacity and size arguments are used by
	...
	public SparseVector(Alphabet dict, PropertyList pl, boolean binary,
		...
		if (pl == null) {
			// xxx Fix SparseVector so that it can properly represent a vector
			// that has all zeros.
			// Does this work?
			indices = new int[0];
			values = null;
		...
		if (binary == false) {
			binary = true;
			// If all the property list features are binary, make a binary
			// SparseVector even if the constructor argument "binary" is false.
			// This will significantly save space, as well as multiplication
			// time later! -akm 12/2007
		...
		while (iter.hasNext()) {
			iter.nextProperty();
			// System.out.println
			// ("SparseVector adding "+iter.getKey()+" "+iter.getNumericValue());
			int index = dict.lookupIndex(iter.getKey(), growAlphabet);
			if (index >= 0) {
				afv.add(index, iter.getNumericValue());
			}
			// System.out.println
			// ("SparseVector afv adding "+iter.getKey()+" afv.numLocations="+afv.numLocations());
		}
		// afv.print();
		// xxx Not so efficient?
		SparseVector sv = afv.toSparseVector();
		// System.out.println
		// ("SparseVector sv.numLocations="+sv.numLocations());
	...
	/**
	 * For each index i that is present in this vector, set this[i] += v[i]. If
	 * v has indices that are not present in this, these are just ignored.
	 */
	...
	/**
	 * For each index i that is present in this vector, set this[i] += factor *
	 * v[i]. If v has indices that are not present in this, these are just
	 * ignored.
	 */
	public void plusEqualsSparse(SparseVector v, double factor) {
		// Special case for dense sparse vector
		if (indices == null) {
			densePlusEqualsSparse(v, factor);
			return;
		}
	...
	public void timesEqualsSparse(SparseVector v, double factor) {
		// Special case for dense sparse vector
		if (indices == null) {
			denseTimesEqualsSparse(v, factor);
			return;
		}
	...
	/**
	 * For each index i that is present in this vector, set this[i] *= factor *
	 * v[i]. If v has indices that are not present in this, these are set to
	 * zero
	 */
	 ...
	 * location as there are in the vector.
	 * 
	...
	/**
	 * Applies the method argument to each value in a non-binary vector. The
	 * method should both accept a Double as an argument and return a Double.
	 * 
	 * @throws IllegalArgumentException
	 *             If the method argument has an inappropriate signature.
	 * @throws UnsupportedOperationException
	 *             If vector is binary
	 * @throws IllegalAccessException
	 *             If the method is inaccessible
	 * @throws Throwable
	 *             If the method throws an exception it is relayed
	 */
	public final void map(Method f) throws IllegalAccessException, Throwable {
		...
		try {
			for (int i = 0; i < values.length; i++)
				values[i] = ((Double) f.invoke(null, new Object[] { new Double(
	...
	public double dotProduct(ConstantMatrix m) {
		if (m instanceof SparseVector)
			return dotProduct((SparseVector) m);
		else if (m instanceof DenseVector)
			return dotProduct((DenseVector) m);
		else
			throw new IllegalArgumentException("Unrecognized Matrix type "
					+ m.getClass());
	}
		...
		if (values == null)
			...
			for (int i = 0; i < indices.length; i++) {
				if (Double.isInfinite(values[i]) && v.value(indices[i]) == 0.0) {
					...
					continue;
				} else if (Double.isInfinite(v.value(indices[i]))
						&& values[i] == 0.0) {
					v.hasInfinite = true;
					continue;
				}
	...
	public double dotProduct(SparseVector v) {
		...
		// This is a heuristic choice based on efficiency, and it could
		// certainly
		// be more complicated.

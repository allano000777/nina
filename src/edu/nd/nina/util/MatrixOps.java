...
import edu.nd.nina.types.FeatureSelection;
import edu.nd.nina.types.FeatureVector;
...
public final class MatrixOps {
	...
	public static final void rowSetAll(double[] m, int nc, int ri, double v,
			FeatureSelection fselection, boolean ifSelected) {
		if (fselection == null) {
			if (ifSelected == true) {
	...
	public static double rowDotProduct(double[] m, int nc, int ri,
			FeatureVector v, int maxCi, FeatureSelection selection) {
		return rowDotProduct(m, nc, ri, v, 1, maxCi, selection);
	}

	public static double rowDotProduct(double[] m, int nc, int ri,
			FeatureVector v, double factor, int maxCi,
	...
	public static final void rowPlusEquals(double[] m, int nc, int ri,
			FeatureVector v, double factor) {
		for (int vli = 0; vli < v.numLocations(); vli++)
			m[ri * nc + v.indexAtLocation(vli)] += v.valueAtLocation(vli)
					* factor;
